...
A<-ggplot(OA_merged, aes(x=e0, y= body_fat)) +
  geom_text(x=0.5, y=55, size=3, label=expression(paste("p = 3.2 x ", 10^-14))) +
  geom_point(size=0.8) + stat_smooth(method='lm', color=pal[1])+
  theme_bw(13) + 
...
ggsave("bodyfat_DC.pdf", A, width=3.5, height=3.5)
...
B<-ggplot(OA_merged, aes(x= (-1 * urb_score_std) - e0, y=body_fat)) +
  ...
  theme(plot.title=element_text(size=14, hjust=0.5))+
  xlab('') + 
  geom_text(x=3, y=55, size=3, label="p = 0.98") +
  ylab("Body fat percentage") +
  ggtitle(""); B

ggsave("bodyfat_PAR.pdf", B, width=3.5, height=3.5)
```
...
```{r}
# List of outcome variables - non std
Body_Height <- c("height_standing", "height_hip", "height_knee")
...
# List of outcome variables - std
Body_Height_std <- c("height_standing_std", "height_hip_std", "height_knee_std")

Body_Press_std <- c("waist_circum_std", "weight_std", "BMI_std", "BRI_std", "body_fat_std", 
                "waist_hip_std","sys_bp2_std", "dias_bp2_std")

Blood_Bio_std <- c("total_chol_std", "ldl_std", "hdl_std", "triglyc_std")

#Other modeling variables

other<-c("sex", "age_std", "urb_score_std", "Early_PC1_std")

all_outcomes<-c("id", other,Body_Height,Body_Press,Blood_Bio,Binary,Body_Height_std,Body_Press_std,Blood_Bio_std)

OA_merged_filtered<-OA_merged %>%
  mutate(id = c(1:nrow(OA_merged))) %>%
  dplyr::select(all_outcomes)
write.csv(OA_merged_filtered, "OA_data_for_peer_review.csv", row.names = F)
```
